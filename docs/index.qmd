...
coviddailynew %>% 
  ...
  theme_vanilla() %>% 
  set_caption(caption = "Colorado Counties with the Most Cumulative Covid Cases as of 02/01/2022")
...
coviddailynew %>% 
  ...
  theme_vanilla() %>% 
  set_caption(caption = "Colorado Counties with the Most New Covid Cases on 02/01/2021")
```
...
covidpop %>%
  ...
  theme_vanilla() %>% 
  set_caption(caption = "Colorado Counties with the Most Cases per Capita on 02/01/2021")
...
covidpop %>%
  ...
  theme_vanilla() %>% 
  set_caption(caption = "Colorado Counties with the Most New Cases per Capita on 02/01/2021")
...
covidpop %>% 
  ...
  theme_vanilla() %>% 
  set_caption(caption = "Five Colorado Counties with the Most New Cases per 100,000 People in the Past 14 days")
...
covidState <- covid %>% 
  ...
  mutate(NewDailyCases = (cases - lag(cases))) %>% 
  group_by(state) %>% 
  transform(rollingMean = rollmean(NewDailyCases, k=7, fill = NA, align = "right")) %>% 
  ungroup()
...
weightedspace %>% 
  filter(date < my.date) %>% 
  ggplot(aes(x=LON,y=LAT,colour = month, size = cases))+
